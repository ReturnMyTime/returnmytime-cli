...
// Zip source tests
test('Zip source - local zip path', () => {
  const result = parseSource('./skills.zip');
  assert.strictEqual(result.type, 'zip');
  assert.ok(result.localPath?.endsWith('skills.zip'));
});

test('Zip source - remote zip url', () => {
  const result = parseSource('https://example.com/skills.zip');
  assert.strictEqual(result.type, 'zip');
  assert.strictEqual(result.url, 'https://example.com/skills.zip');
});
